...
<?php include("dbcon.php");
  ...
  $query_tracks = $pdo->prepare("SELECT music.title AS title, artists.name AS artist, music.releaseDate AS releaseDate, music.source AS source FROM music LEFT JOIN artists ON music.artistId=artists.id WHERE artistId=:artistId");
  $query_tracks->bindParam("artistId", $_GET['id'], PDO::PARAM_INT);
  $query_tracks->execute();
  $tracks = $query_tracks->fetchAll(PDO::FETCH_ASSOC);

  $query_videos = $pdo->prepare("SELECT videos.title AS title, artists.name AS artist, videos.releaseDate AS releaseDate, videos.source AS source FROM videos LEFT JOIN artists ON videos.artistId=artists.id WHERE artistId=:artistId");
  $query_videos->bindParam("artistId", $_GET['id'], PDO::PARAM_INT);
...
<script>
    document.getElementById('nav_artists').classList.add('active');
</script>
