...
<?php
  if (isset($_POST['editBtn'])){
    $query = $pdo->prepare('update music set title=:title, artistId=:artistId, genreId=:genreId, languageId=:languageId, releaseDate=:releaseDate, description=:description, source=:source where id=:id');
    ...
    if (isset($_FILES['audioFile'])) {
      ...
      $src = $_FILES["audioFile"]["name"];

    } else {
      $src = $_POST['audio_old'];
    }
    $query->bindParam('source', $src, PDO::PARAM_STR);
...
<!-- Fetch Query -->
<?php
  $query = $pdo->query("SELECT music.id, music.title, artists.name AS artist, albums.name AS album, genres.name AS genre, languages.language AS language, music.releaseDate, music.description, music.source FROM music INNER JOIN artists ON music.artistId = artists.id LEFT JOIN albums ON music.albumId =albums.id INNER JOIN genres ON music.genreId = genres.id INNER JOIN languages ON music.languageId = languages.id");
...
<?php
  if (isset($_GET['action']) && $_GET['action'] == 'fetch') {
    $query = $pdo->prepare('select * from music where id=:id');
    $query->bindParam('id', $_GET['id'], PDO::PARAM_INT);
    $query->execute();
    $music_edit = $query->fetch(PDO::FETCH_ASSOC);
  }
?>
<!-- /Fetch query -->
...
<?php
  if (isset($_POST['addNew'])) {
    ...
    $query = $pdo->prepare('insert into music(title, artistId, genreId, languageId, releaseDate, description, source) values(:title, :artistId, :genreId, :languageId, :releaseDate, :description, :source)');
    $query->bindParam('title', $_POST['title'], PDO::PARAM_STR);
    ...
    $query->bindParam('releaseDate', $_POST['releaseDate'], PDO::PARAM_STR);
    $query->bindParam('description', $_POST['description'], PDO::PARAM_STR);
    $query->bindParam('source', $_FILES["audioFile"]["name"], PDO::PARAM_STR);
    $query->execute();
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLongTitle">Edit Music</h5>
        <button type="button" onclick="window.location.href='music.php'" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form class="" action="music.php" method="post" enctype="multipart/form-data">
        <div class="modal-body">
          <input type="hidden" name="id" value="<?php echo $music_edit['id']; ?>">
          
          <div class="form-group">
            <label>Title</label>
            <input class="form-control" type="text" name="title" value="<?php echo ucwords($music_edit['title']); ?>">
          </div>
          ...
          <div class="form-group">
            ...
            <select required name="artistId" class="form-control">
              ...
              <?php 
                ...
                ?>
                  <option <?php if ($music_edit['artistId'] == $item['id']) { echo "selected"; } ?> value="<?php echo $item["id"]; ?>"><?php echo ucwords($item["name"]); ?></option>
                <?php
                }
                ?>
            </select>
          ...
          <div class="form-group">
            ...
            <select name="albumId" class="form-control">
              ...
              ?>
              <option <?php if ($music_edit['albumId'] == $item['id']) { echo "selected"; } ?> value="<?php echo $item["id"]; ?>"><?php echo ucwords($item["name"]); ?></option>
              <?php
              }
              ?>
            </select>
          ...
          <div class="form-group">
            ...
            <select required name="genreId" class="form-control">
              ...
              ?>
              <option <?php if ($music_edit['genreId'] == $item['id']) { echo "selected"; } ?> value="<?php echo $item["id"]; ?>"><?php echo ucwords($item["name"]); ?></option>
              <?php
              }
              ?>
            </select>
          ...
          <div class="form-group">
            ...
            <select required name="languageId" class="form-control">
              ...
              ?>
              <option <?php if ($music_edit['languageId'] == $item['id']) { echo "selected"; } ?> value="<?php echo $item["id"]; ?>"><?php echo ucwords($item["language"]); ?></option>
              <?php
              }
              ?>
            </select>
          ...
          <div class="form-group">
            ...
            <input required class="form-control" type="date" name="releaseDate" value="<?php echo strftime('%Y-%m-%d', strtotime($music_edit['releaseDate'])); ?>">
          </div>  
          
          <!-- description -->
          <div class="form-group">
            <label>Description</label>
            <input class="form-control" type="text" name="description" value="<?php echo $music_edit['description']; ?>">
          </div>
          ...
          <div class="form-group">
            ...
            <input type="text" name="audio_old" hidden value="<?php echo $music_edit['source']; ?>">
          </div>

        </div>
        <div class="modal-footer">
          <button type="button" onclick="window.location.href='music.php'" class="btn btn-secondary" data-dismiss="modal">Close</button>
        ...
        <tbody>
          ...
          <form class="" action="" method="post" enctype="multipart/form-data">
            <tr>
              ...
              <!-- audio file -->
              <td>
                <input onchange="checkFile(this)" required class="form-control" type="file" accept=".mp3" name="audioFile">
                <br/>
                <div align="center">
    ...
    <tbody>
      ...
      <?php foreach ($music as $item) { ?>
          <tr>
            ...
            <td><a href="music.php?action=fetch&id=<?php echo $item['id']; ?>">Edit</a> | 
            <a href="music.php?action=delete&id=<?php echo $item['id']; ?>">Delete</a></td>
          </tr>
      <?php } ?>
    </tbody>
  </table>
